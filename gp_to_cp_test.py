...
class maskstocellpose_flows(gp.BatchFilter):
  ...
  def __init__(self, in_array, out_array, is_2D = False):
...
simple_augment = gp.SimpleAugment(mirror_probs=[0,1,1], transpose_probs=[0,1,1])
stack = gp.Stack(5)
...
source = tuple(gp.ZarrSource(
    ...
    },
    {
      ...
      gt: gp.ArraySpec(interpolatable=False,
        voxel_size=(1,1,1)),
    }) + normalize + random_location for i in [1]) #pad_raw + pad_gt  +
#%%
